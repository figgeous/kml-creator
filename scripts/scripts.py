...
def dataframe_to_shp(*, input_df, output_file_path: str = None) -> gp.GeoDataFrame:
    ...
    gpdf = gp.GeoDataFrame(input_df, geometry="geometry")
    if output_file_path:
...
def get_pandas_from_csv(
    *, csv_name: str, sep: str = ",", index_col: str = None
) -> pd.DataFrame:
...
def pandas_to_csv(
    *, df: pd.DataFrame, csv_name: str, sep: str = ",", index_col: str = None
) -> pd.DataFrame:
    assert ".csv" not in csv_name
